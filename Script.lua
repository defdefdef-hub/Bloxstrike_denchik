-- DeNsI script xexex (PasteHub + Custom GUI)
...
for _, g in ipairs(LP:WaitForChild("PlayerGui"):GetChildren()) do
    if g.Name:sub(1,3) == "DC_" or g.Name:sub(1,6) == "DeNsI_" then g:Destroy() end
end
...
local Cfg = {
    ...
    ESPBoxOutline=Color3.new(0,0,0),
    ESPBoxFillGradient=false, ESPFillColorA=Color3.fromRGB(255,50,50), ESPFillColorB=Color3.fromRGB(50,50,255),
    ...
    ESPTracer=false, ESPTracerColor=Color3.new(1,1,1), ESPTracerColorB=Color3.fromRGB(255,0,128), ESPTracerOrigin="Bottom",
    ...
    SilentWallbang=false, SilentUseFovCircle=false, SilentFovColor=Color3.fromRGB(255,0,0), SilentFovRadius=50,
    -- Effects
    ...
    HitMarkerDuration=2, HitMarkerSpinSpeed=720, HitMarkerSize=25, HitMarkerThickness=2,
    ...
    -- Memesense (Cube)
    MemesenseMainToggle=false, CubeAimbotEnabled=false, CubeVisibleCheck=false,
    CubeHitPart="Head", CubeTriggerbot=false, CubeTriggerbotDelay=0.01,
    BulletImpactV1Enabled=false, BulletImpactV1Color=Color3.fromRGB(255,0,0), BulletImpactV1Rainbow=false,
    BulletImpactV1Size=1.5, BulletImpactV1Dist=20,
    ShowTargetPlayer=false, ShowTargetMode="Crosshair",
    ShowTargetLineColor=Color3.fromRGB(0,255,255), ShowTargetCrosshairColor=Color3.fromRGB(0,255,255),
    -- Ragebot
    Ragebot=false, RageDelay=0.01, RageHitPart="Head",
    RagebotVisibleCheck=true, RagebotTeamCheck=true, RagebotWallCheck=false,
    -- GUI
...
local function get_player_team(p)
    if not p or not p.Team then return nil end
    return p.Team.Name
end
...
local function PlayHitSound()
    pcall(function()
        ...
        local sid = ""
        if Cfg.CustomHitSoundToggle and Cfg.CustomHitSoundID ~= "" then
            local ci = Cfg.CustomHitSoundID
            if not ci:find("rbxassetid://") then
                local clean = ci:gsub("%D","")
                if clean ~= "" then sid = "rbxassetid://"..clean end
            else sid = ci end
        end
        if sid == "" then sid = HitSoundPresets[Cfg.HitSoundPreset] or HitSoundPresets.Neverlose end
        local s = Instance.new("Sound")
        s.SoundId = sid; s.Volume = Cfg.HitSoundVolume or 1
...
local function TriggerHitMarker(hitPos)
    if not Cfg.HitMarkerEnabled then return end
    local lines = {}
    for i = 1, 4 do
        local l = Drawing.new("Line")
        l.Thickness = Cfg.HitMarkerThickness; l.Transparency = 1; l.Visible = false
        lines[i] = l
    end
    table.insert(activeHitMarkers, {
        lines=lines, worldPos=hitPos, spawnTick=tick(),
        expireTick=tick()+Cfg.HitMarkerDuration
    })
end
task.spawn(function()
    while task.wait() do
        local now = tick()
        local col = Cfg.HitMarkerColor
        if Cfg.HitMarkerRainbow then col = Color3.fromHSV((now%5)/5,1,1) end
        local pf = 1 + 0.35*math.sin(now*math.pi)
        local cs = Cfg.HitMarkerSize * pf
        local gap = 6 * pf
        for i = #activeHitMarkers, 1, -1 do
            local d = activeHitMarkers[i]
            if not Cfg.HitMarkerEnabled or now > d.expireTick then
                ...
                if on and sp.Z > 0 then
                    ...
                    local a = math.rad((lt * Cfg.HitMarkerSpinSpeed) % 360)
                    for j = 1, 4 do
                        local l = d.lines[j]
                        l.Color = col; l.Thickness = Cfg.HitMarkerThickness
...
-- ========== ANTI-FLASH / ANTI-SMOKE (GC hooks) ==========
pcall(function()
    for _, obj in next, getgc(true) do
...
-- ========== WEAPON CHAMS ==========
local activeWeaponHLs = {}
RunService.RenderStepped:Connect(function()
    pcall(function()
        local en = Cfg.WeaponChamsEnabled
        local mode = Cfg.WeaponChamsMode
        local color = Cfg.WeaponChamsColor
        local wm = nil
        for _, ch in ipairs(Cam:GetChildren()) do
        ...
        if not en or not wm then
            for _, h in pairs(activeWeaponHLs) do if h and h.Parent then h:Destroy() end end
            activeWeaponHLs = {}; return
        end
        local curParts = {}
                ...
                pcall(function()
                    if mode == "Highlight" then
                        ...
                        if not h then
                            ...
                            table.insert(activeWeaponHLs, h)
                        end
                        h.FillColor = color
                    else
                        local h = part:FindFirstChild("WeaponChamsHighlight")
                        ...
                        if mode == "Glass" then part.Material=Enum.Material.Glass; part.Color=color; part.Transparency=Cfg.GlassTransparency
                        elseif mode == "ForceField" then part.Material=Enum.Material.ForceField; part.Color=color; part.Transparency=0
                        elseif mode == "Metal" then part.Material=Enum.Material.Metal; part.Color=color; part.Reflectance=Cfg.MetalReflectance; part.Transparency=0
        ...
        if mode == "Highlight" then
            for i = #activeWeaponHLs, 1, -1 do
                local h = activeWeaponHLs[i]
                if not h or not h.Parent or not curParts[h.Adornee] then
                    if h then h:Destroy() end
                    table.remove(activeWeaponHLs, i)
                end
            end
        end
    end)
...
task.spawn(function()
    ...
    pcall(function()
        ...
        ESPFolder.Name = "DeNsI_Chams_Container"
    end)
    local Highlights = {}
    ...
    RunService.RenderStepped:Connect(function()
        local en = Cfg.ChamsEnabled
        local tcOn = Cfg.ChamsTeamCheck
        local mv = getMat(Cfg.ChamsMaterialVisible)
        local mu = getMat(Cfg.ChamsMaterialUnvisible)
        local cv = Cfg.ChamsColorVisible
        local cu = Cfg.ChamsColorUnvisible
        local fv = Cfg.ChamsAlphaVisible
        local fu = Cfg.ChamsAlphaUnvisible
        local ov = Cfg.ChamsOutlineAlphaVisible
        local ou = Cfg.ChamsOutlineAlphaUnvisible
...
-- ========== CAMERA (FOV / 3rd Person / Scope / Custom Hands) ==========
RunService.RenderStepped:Connect(function()
    pcall(function()
...
task.spawn(function()
    ...
    sg.Name = "DeNsI_ScopeCrosshair"; sg.ResetOnSpawn = false
...
local function UpdateSkybox(name)
    ...
    if not sky then
        ...
        sky = Instance.new("Sky"); sky.Name = "DeNsI_Sky"; sky.Parent = Lighting
    end
...
local function UpdateWeather(t)
    ...
    WeatherPart.Name="DeNsI_Weather"; WeatherPart.Size=Vector3.new(100,1,100)
...
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            if Cfg.EnableTime then Lighting.ClockTime = Cfg.WorldClockTime
            else Lighting.ClockTime = DefaultLighting.ClockTime end
            if Cfg.EnableBrightness then Lighting.Brightness = Cfg.WorldBrightness
            else Lighting.Brightness = DefaultLighting.Brightness end
            if Cfg.EnableColors then
                Lighting.Ambient = Cfg.WorldAmbient
                Lighting.OutdoorAmbient = Cfg.WorldOutdoorAmbient
            else
...
local function IsLocalObject(obj)
    ...
    if obj.Name:find("DC_") or obj.Name:find("DeNsI_") or obj.Name == "BS_RingSeg" then return true end
    return false
end
...
RunService.RenderStepped:Connect(function()
    if WorldSettings.SkyColorEnabled then
        ...
        Lighting.ColorShift_Bottom = c; Lighting.ColorShift_Top = c
        Lighting.FogColor = c
        local atm = Lighting:FindFirstChild("DeNsI_Atm")
        if not atm then
            atm = Instance.new("Atmosphere"); atm.Name = "DeNsI_Atm"; atm.Parent = Lighting
        end
...
-- ========== GRENADE ZONE ESP ==========
if _G.DeNsI_ZoneLoaded then
    warn("[ZoneESP] Already running")
else
    _G.DeNsI_ZoneLoaded = true
    local SETTINGS = {
    ...
    local function MakeCyl(col, thick)
        local p = Instance.new("Part")
        p.Name="DeNsI_RingSeg"; p.Anchored=true; p.CanCollide=false; p.CanQuery=false
...
-- =========================================================================
-- [ ЧАСТЬ 3 — ESP / AIM / SKINS / RAGEBOT / TRIGGERBOT / CUBE / WEAPON / GUI ]
-- =========================================================================

-- ========== PASTEHUB ESP (2D/3D/Corner Box + Skeleton + Tracer + Circular) ==========
...
local sin, cos, pi2 = math.sin, math.cos, math.pi*2
...
local function project_fixed_box(x0,y0,z0,x1,y1,z1)
    ...
    local sp,vis=W2VP(Cam,Vector3.new(cx,cy,cz))
    if not vis and sp.Z <= 0 then return nil,nil,false end
    local depth=sp.Z; if depth<=0 then depth = 0.1 end
    local scale=BASE_DIST/depth
    local w=BASE_W*scale; local h=BASE_H*scale
...
local function project_aabb_corners_3d(x0,y0,z0,x1,y1,z1)
    ...
    for i=1,8 do
        ...
        local pos,vis=W2VP(Cam,Vector3.new(wx,wy,wz))
...
local function drawFillGradient360(fillLines, x, y, w, h, colorA, colorB, angle)
    ...
    for i = 1, targetRows do
        ...
        local tL = clamp(dotL*0.5+0.5,0,1); local tR = clamp(dotR*0.5+0.5,0,1)
        local line = fillLines[i]
        line.Color = lerpColor(colorA, colorB, (tL+tR)*0.5)
        line.Thickness = clamp(rowH+1.5, 2, 8)
        line.From = Vector2.new(x+1, py); line.To = Vector2.new(x+w-1, py)
        line.Visible = true
    end
...
local function esp_add_healthbar(instance)
    ...
    local segs={}; for i=1,MAX_HP_SEGMENTS do segs[i]=Drawing.new("Line"); segs[i].Thickness=3; segs[i].Transparency=1; segs[i].Visible=false end
    espinstances[instance]=espinstances[instance] or {}
...
local function esp_add_healthtext(instance)
    ...
    local t=Drawing.new("Text"); t.Center=false; t.Outline=true; t.Font=1; t.Transparency=1; t.Visible=false
    espinstances[instance]=espinstances[instance] or {}; espinstances[instance].healthtext=t
end
...
local function esp_add_circulartarget(instance)
    ...
    local TRAIL_SEGS=25; local trailLines={}; local neonGlowLines={}
    for i=1,TRAIL_SEGS do
        local l=Drawing.new("Line"); l.Thickness=2.5; l.Transparency=0.4; l.Visible=false; trailLines[i]=l
        local g=Drawing.new("Line"); g.Thickness=5.0; g.Transparency=0.15; g.Visible=false; neonGlowLines[i]=g
    end
    espinstances[instance]=espinstances[instance] or {}
    espinstances[instance].circulartarget={lines=lines,trailLines=trailLines,neonGlowLines=neonGlowLines,segments=SEGS,alpha=0,movingUp=true,trailHistory={}}
end
...
local function get_cached_screen_pos(cache,part)
    ...
    local pos,vis=W2VP(Cam,part.Position)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    for instance,data in next,espinstances do
        ...
        if teamCheck then
            ...
            local isAlly = myHasVest and not targetHasVest or (not myHasVest and targetHasVest)
        ...
        if data.box then
            if needBox and onscreen and min2 and max2 then
                ...
                local cA, cB = Cfg.ESPBoxColorA, Cfg.ESPBoxColorB
                local fA, fB = Cfg.ESPFillColorA, Cfg.ESPFillColorB
            ...
            if needHp and onscreen and min2 and max2 and healthAttr then
                ...
                for i = 1, MAX_HP_SEGMENTS do
                    ...
                    if i <= activeSegCount then
                        ...
                        segLine.Color = lerpColor(Cfg.ESPHealthBottomColor, Cfg.ESPHealthTopColor, segFrac)
        ...
        if data.tracer then
            if needTracer and onscreen and min2 and max2 then
                local from_pos
                local originMode = Cfg.ESPTracerOrigin or "Bottom"
        ...
        if data.circulartarget then
            ...
            if needCirc and head and root then
                ...
                for i = 1, #ct.trailLines do
                    local trailLine = ct.trailLines[i]; local glowLine = ct.neonGlowLines[i]
                    local p1 = ct.trailHistory[i]; local p2 = ct.trailHistory[i+1]
                    if p1 and p2 then
                        local s1,v1 = W2VP(Cam, p1); local s2,v2 = W2VP(Cam, p2)
                        if v1 or v2 then
...
local function addEspToCharacter(character)
    ...
    esp_add_box(character); esp_add_name(character); esp_add_healthbar(character)
    esp_add_healthtext(character); esp_add_distance(character); esp_add_tracer(character)
    esp_add_skeleton(character,{thickness=2}); esp_add_weapon(character); esp_add_circulartarget(character)
    espCharacters[character]=true
end
local function removeEspFromCharacter(character)
    if character then
        if espinstances[character] then cleanup_instance(character, espinstances[character]); espinstances[character]=nil end
...
if charactersFolder then
    charactersFolder.DescendantAdded:Connect(function(d)
        if d:IsA("Model") then
            task.wait(0.1)
            if d:GetAttribute("Health") ~= nil or d:FindFirstChild("Head") then addEspToCharacter(d) end
        end
    end)
    charactersFolder.DescendantRemoving:Connect(function(d)
        if d:IsA("Model") then removeEspFromCharacter(d) end
    end)
end
...
local AimRaycast, AimGetRayIgnore
local function selectTarget(cam, origin, maxRange, randomize)
    ...
    local ignore = Cfg.SilentVisibleOnly and AimGetRayIgnore and AimGetRayIgnore() or nil
    ...
    for _, cand in ipairs(cands) do
        ...
        local visible = true
        if Cfg.SilentVisibleOnly and AimRaycast then
            local off = cand.Position - origin
            local hit = AimRaycast.cast(origin, off.Unit * (off.Magnitude + 0.05), nil, ignore)
...
local function redirectShot(bullet, shot, target)
    ...
    local ignore = AimGetRayIgnore and AimGetRayIgnore()
    local res = {Origin=shot.Origin, Direction=dir, Distance=props.Range or 500, Hits={}}
    local first = AimRaycast.cast(shot.Origin, dir * res.Distance, nil, ignore)
    if first.instance then
        res.Distance = (first.position - shot.Origin).Magnitude
        local hits = AimRaycast.castThrough(first.position - dir * 0.001, dir * (pen + 0.001), pen, ignore)
...
local function installSilent()
    pcall(function()
        ...
        Aim.Module, Aim.Original = bullet, bullet._performRaycast
        AimRaycast, AimGetRayIgnore = raycast, getIgnore
...
fovStroke.Color = Cfg.GuiColor; fovStroke.Thickness = 1.5; fovStroke.Transparency = 0.3; fovStroke.Parent = fovCircle
...
local ringStroke = Instance.new("UIStroke"); ringStroke.Color = Cfg.GuiColor; ringStroke.Thickness = 3; ringStroke.Parent = targetRing
...
RunService.RenderStepped:Connect(function()
    ...
    if lt and lt.Part and lt.Part.Parent and lt.Character and lt.Character.Parent and Cfg.SilentShowTarget then
        ...
        if onScreen and sp.Z > 0 then
            targetRing.Position = UDim2.new(0, sp.X, 0, sp.Y); targetRing.Visible = true
...
if SD.SkinsRoot then
    pcall(function()
        for _, wf in ipairs(SD.SkinsRoot:GetChildren()) do
            ...
            table.sort(skins); SD.SkinSelections[wf.Name] = skins
        end
...
local function InitSkinHooks()
    ...
    pcall(function()
        ...
        if Sk.GetCameraModel then
            local o = Sk.GetCameraModel
            Sk.GetCameraModel = function(w, sk, ...)
                if Cfg.KnifeChangerEnabled and w and Checkknife(w) then
                    local r = o(Cfg.KnifeChangerModel, Cfg.SkinChangerSkins[Cfg.KnifeChangerModel] or "Vanilla", ...)
        ...
        if Vm.new then
            ...
            Vm.new = function(vc, w, sk, ...)
                if Cfg.KnifeChangerEnabled and w and Checkknife(w) then
                    local r = o(vc, Cfg.KnifeChangerModel, Cfg.SkinChangerSkins[Cfg.KnifeChangerModel] or "Vanilla", ...)
                    if r then return r end
                end
                return o(vc, w, sk, ...)
            end
        ...
        if Sk.GetGloves then
            local o = Sk.GetGloves
            Sk.GetGloves = function(g, sk)
                if Cfg.GloveChangerEnabled and Cfg.GloveChangerModel then
                    local r = o(Cfg.GloveChangerModel, Cfg.GloveChangerGloves[Cfg.GloveChangerModel] or "Default")
                    if r then return r end
                end
                return o(g, sk)
            end
...
local function ApplyGloves()
    ...
    local sm = Cfg.GloveChangerModel; if not sm then return end
    local sel = Cfg.GloveChangerGloves[sm]; if not sel or sel == "Default" then return end
    local gf = SD.SkinsRoot:FindFirstChild(sm); if not gf then return end
    local sv = gf:FindFirstChild(sel); if not sv then return end
    local cf = sv:FindFirstChild("Camera"); if not cf then return end
    local fn = cf:FindFirstChild("Factory New"); if not fn then return end
    for _, sa in pairs(fn:GetChildren()) do
        if sa:IsA("SurfaceAppearance") then
            sa:Clone().Parent = lg; sa:Clone().Parent = rg
        end
    end
end
task.spawn(function()
...
-- ========== FIRERATE / NORECOIL / NOSPREAD / INSTANT RELOAD ==========
...
pcall(function()
    for _, obj in next, getgc(true) do
        ...
        if type(obj) == "table" and rawget(obj, "setWeaponRecoil") then
            pcall(function() local old; old = hookfunction(obj.setWeaponRecoil, function(...) if Cfg.NoRecoil then return end return old(...) end) end)
        end
        if type(obj) == "function" and debug.getinfo(obj).name == "calculateRecoilOffset" then
            pcall(function() local old; old = hookfunction(obj, function(...) if Cfg.NoRecoil then return UDim2.new() end return old(...) end) end)
        end
        if type(obj) == "table" and rawget(obj, "weaponKick") then
            pcall(function() local old; old = hookfunction(obj.weaponKick, function(p1,p2) if Cfg.NoRecoil then return end return old(p1,p2) end) end)
        end
        if type(obj) == "table" and rawget(obj, "getTrueSpread") then
            pcall(function() local old; old = hookfunction(obj.getTrueSpread, function(p1) if Cfg.NoSpread then return 0 end return old(p1) end) end)
        end
            ...
            if Cfg.Firerate then
                for _, obj in next, firerateObjs do
                    pcall(function()
                        ...
                        rawset(obj, "FireRate", math.max(Cfg.FirerateSlider, 0.01))
...
task.spawn(function()
    while true do
        task.wait(Cfg.RageDelay or 0.02)
        pcall(function()
            if not Cfg.Ragebot then return end
            local cam = workspace.CurrentCamera; if not cam then return end
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LP then
                    local c = matchChar(p)
                    if c and (not Cfg.RagebotTeamCheck or teamOf(p) ~= myTeam) then
                        local part = c:FindFirstChild(Cfg.RageHitPart) or c:FindFirstChild("Head") or c:FindFirstChild("HumanoidRootPart")
                        if part then
            ...
            RageTarget = closest
            if RageTarget then
                local weapon = getCurrentEquipped and select(1, pcall(function() return debug.getupvalue(getCurrentEquipped, 1).CurrentEquipped end)) or nil
                if weapon and weapon.IsEquipped and weapon.Rounds > 0 then weapon:shoot() end
            end
        end)
    end
end)
...
-- ========== TRIGGERBOT (Cube) ==========
task.spawn(function()
    local trigRay = RaycastParams.new()
    trigRay.FilterType = Enum.RaycastFilterType.Exclude
    while true do
        task.wait(Cfg.CubeTriggerbotDelay or 0.01)
        pcall(function()
            if not Cfg.CubeTriggerbot or not Cfg.MemesenseMainToggle then return end
            local cam = workspace.CurrentCamera; if not cam then return end
            trigRay.FilterDescendantsInstances = {LP.Character}
            local result = workspace:Raycast(cam.CFrame.Position, cam.CFrame.LookVector*1000, trigRay)
            if result and result.Instance then
                local char = result.Instance:FindFirstAncestorOfClass("Model")
                if char then
                    local p = Players:GetPlayerFromCharacter(char)
                    if p and p ~= LP and teamOf(p) ~= teamOf(LP) then
                        local w = getCurrentEquipped and debug.getupvalue(getCurrentEquipped, 1).CurrentEquipped or nil
                        if w and w.IsEquipped and w.Rounds > 0 then w:shoot() end
                    end
                end
            end
        end)
...
-- ========== CUBE CHECKER (визуальный) ==========
local CubePart = Instance.new("Part")
CubePart.Name = "DeNsI_CubeChecker"; CubePart.Size = Vector3.new(1.5,1.5,0.01)
CubePart.Anchored = true; CubePart.CanCollide = false; CubePart.CanQuery = false; CubePart.CanTouch = false
CubePart.Material = Enum.Material.Neon; CubePart.Transparency = 0.98; CubePart.Parent = workspace
local cubeSel = Instance.new("SelectionBox"); cubeSel.Adornee = CubePart; cubeSel.LineThickness = 0.04; cubeSel.Transparency = 0.2; cubeSel.Parent = CubePart
local cubeRay = RaycastParams.new(); cubeRay.FilterType = Enum.RaycastFilterType.Exclude; cubeRay.IgnoreWater = true
RunService.RenderStepped:Connect(function()
    pcall(function()
        if not Cam then return end
        local en = Cfg.MemesenseMainToggle and Cfg.BulletImpactV1Enabled
        CubePart.Parent = en and workspace or nil
        if en then
            CubePart.Size = Vector3.new(Cfg.BulletImpactV1Size, Cfg.BulletImpactV1Size, 0.01)
            local col = Cfg.BulletImpactV1Color
            if Cfg.BulletImpactV1Rainbow then col = Color3.fromHSV((tick()%5)/5,1,1) end
            cubeRay.FilterDescendantsInstances = {LP.Character, CubePart}
            local r = workspace:Raycast(Cam.CFrame.Position, Cam.CFrame.LookVector*Cfg.BulletImpactV1Dist, cubeRay)
            if r then
                CubePart.CFrame = CFrame.lookAt(r.Position + r.Normal*0.02, r.Position + r.Normal)
                CubePart.Color = col; cubeSel.Color3 = col
...
local function applyTheme()
    ...
    for _, it in ipairs(themeRefs) do
        ...
        if o and o.Parent then
            ...
            elseif it.role == "scroll" then o.ScrollBarImageColor3 = ac end
        end
    end
end
...
local function buildGUI()
    ...
    reopenBtn.Size = UDim2.new(0,34,0,34); reopenBtn.Position = UDim2.new(0,14,0,70)
    reopenBtn.BackgroundColor3 = Color3.fromRGB(20,20,24); reopenBtn.BorderSizePixel = 0
    reopenBtn.Text = "+"; reopenBtn.TextColor3 = Color3.fromRGB(200,200,220)
    reopenBtn.Font = Enum.Font.GothamBold; reopenBtn.TextSize = 18
    reopenBtn.Visible = false; reopenBtn.AutoButtonColor = false; reopenBtn.Parent = gui
    ...
    main.Active = true; main.Draggable = true; main.ClipsDescendants = true; main.Parent = gui
    reg(main, "bgMain")
    ...
    header.Size = UDim2.new(1,0,0,38); header.BackgroundColor3 = Color3.fromRGB(20,20,24)
    ...
    titleLbl.BackgroundTransparency = 1; titleLbl.Text = "DeNsI script xexex"
    ...
    divider.BackgroundColor3 = Color3.fromRGB(45,45,55); divider.BorderSizePixel = 0; divider.Parent = content
    reg(divider, "stroke")
    ...
    local function switchTab(name)
        ...
        for n, b in pairs(tabButtons) do
            ...
            local bar = b:FindFirstChild("ActiveBar"); if bar then bar.Visible = active end
    ...
    local function makeTabBtn(name)
        ...
        b.Size = UDim2.new(1,0,0,26); b.BackgroundColor3 = Color3.fromRGB(20,20,24)
        b.BorderSizePixel = 0; b.Text = ""; b.AutoButtonColor = false; b.Parent = sidebar
        local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0,6); c.Parent = b
        local bar = Instance.new("Frame")
        bar.Name = "ActiveBar"; bar.Size = UDim2.new(0,3,0,14); bar.Position = UDim2.new(0,0,0.5,-7)
        bar.BackgroundColor3 = Cfg.GuiColor; bar.BorderSizePixel = 0; bar.Visible = false; bar.Parent = b
        reg(bar, "accent")
    ...
    local function slider(parent, txt, key, mn, mx, step)
        ...
        local function upd(i)
            ...
            Cfg[key] = v; val.Text = tostring(v); fill.Size = UDim2.new(rel, 0, 1, 0)
        end
    ...
    local function optionRow(parent, txt, key, opts, cb)
        ...
        holder.ScrollBarThickness = 0; holder.CanvasSize = UDim2.new(0, #opts*57, 0, 0)
    ...
    local function colorRow(parent, lblTxt, setter)
        ...
        row2.ScrollBarThickness = 0; row2.CanvasSize = UDim2.new(0, #PALETTE*28, 0, 0)
    ...
    colorRow(visualTab, "Цвет меню", function(c)
        ...
        stripe.BackgroundColor3 = c; fovStroke.Color = c
    end)
    ...
    toggle(trTab, "Rainbow", "TracerRainbow")
    ...
    toggle(trTab, "Попадания", "BulletImpacts")
    colorRow(trTab, "Цвет", function(c) Cfg.BulletImpactsColor = c end)
    ...
    slider(camTab, "Толщина", "ScopeCrosshairThickness", 1, 10, 1)
    ...
    toggle(chTab, "Включить", "ChamsEnabled")
    toggle(chTab, "Скрыть союзников", "ChamsTeamCheck")
    section(chTab, "Видимые")
    optionRow(chTab, "Материал", "ChamsMaterialVisible", {"Neon","Metal","ForceField","SmoothPlastic"})
    slider(chTab, "Прозрачность", "ChamsAlphaVisible", 0, 1, 0.05)
    slider(chTab, "Контур", "ChamsOutlineAlphaVisible", 0, 1, 0.05)
    colorRow(chTab, "Цвет", function(c) Cfg.ChamsColorVisible = c end)
    section(chTab, "Невидимые")
    optionRow(chTab, "Материал", "ChamsMaterialUnvisible", {"Neon","Metal","ForceField","SmoothPlastic"})
    slider(chTab, "Прозрачность", "ChamsAlphaUnvisible", 0, 1, 0.05)
    slider(chTab, "Контур", "ChamsOutlineAlphaUnvisible", 0, 1, 0.05)
    colorRow(chTab, "Цвет", function(c) Cfg.ChamsColorUnvisible = c end)
    section(chTab, "Weapon Chams")
    toggle(chTab, "Chams оружия", "WeaponChamsEnabled")
    optionRow(chTab, "Материал", "WeaponChamsMode", {"Glass","ForceField","Metal","Highlight","Neon"})
    slider(chTab, "Glass", "GlassTransparency", 0, 1, 0.05)
    slider(chTab, "Metal", "MetalReflectance", 0, 1, 0.1)
    colorRow(chTab, "Цвет оружия", function(c) Cfg.WeaponChamsColor = c end)
    ...
    toggle(wTab, "World Color", "WorldColorToggle", function(v)
        WorldSettings.WorldColorEnabled = v; RefreshWorld()
    end)
    colorRow(wTab, "World Color", function(c)
        Cfg.WorldColorPicker = c; WorldSettings.WorldColor = c
    ...
    colorRow(wTab, "Second Color", function(c)
        Cfg.SkyColorPicker = c; WorldSettings.SkyColor = c
    end)

    -- WEAPON
    local wpnTab = makeTab("WEAPON")
    section(wpnTab, "Стрельба")
    toggle(wpnTab, "Firerate", "Firerate")
    slider(wpnTab, "Firerate", "FirerateSlider", 0, 1, 0.01)
    toggle(wpnTab, "No Recoil", "NoRecoil")
    toggle(wpnTab, "No Spread", "NoSpread")
    section(wpnTab, "Memesense")
    toggle(wpnTab, "Memesense Mode", "MemesenseMainToggle")
    toggle(wpnTab, "Cube Checker", "BulletImpactV1Enabled")
    toggle(wpnTab, "Cube Rainbow", "BulletImpactV1Rainbow")
    slider(wpnTab, "Размер", "BulletImpactV1Size", 0.5, 4, 0.5)
    slider(wpnTab, "Дистанция", "BulletImpactV1Dist", 1, 50, 1)
    colorRow(wpnTab, "Цвет куба", function(c) Cfg.BulletImpactV1Color = c end)
    toggle(wpnTab, "Triggerbot", "CubeTriggerbot")
    slider(wpnTab, "Delay", "CubeTriggerbotDelay", 0, 1, 0.01)
    ...
    section(miscTab, "Ragebot")
    toggle(miscTab, "Ragebot", "Ragebot")
    slider(miscTab, "Delay", "RageDelay", 0, 1, 0.01)
    optionRow(miscTab, "Hit Part", "RageHitPart", {"Head","HumanoidRootPart","UpperTorso","LowerTorso"})
    toggle(miscTab, "Visible Check", "RagebotVisibleCheck")
    toggle(miscTab, "Team Check", "RagebotTeamCheck")
    ...
    local function minimize()
        ...
        task.wait(0.22); main.Visible = false; reopenBtn.Visible = true
    ...
    local function maximize()
        ...
        isMin = false; reopenBtn.Visible = false; main.Visible = true
...
print("[DeNsI script xexex] ESP: OK")
print("[DeNsI script xexex] Aim: " .. (Aim.Ready and "OK" or "FAIL"))
print("[DeNsI script xexex] Skins: " .. (SD.SkinsRoot and "OK" or "Skins не найдены"))
print("[DeNsI script xexex] Загружено успешно")
